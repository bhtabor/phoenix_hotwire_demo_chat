defmodule PhoenixHotwireDemoChatWeb.MessageController do
  ...
  def create(conn, %{"room_id" => room_id, "message" => message_params}) do
    case Chat.create_message(Map.merge(message_params, %{"room_id" => room_id})) do
      {:ok, _message} ->
        broadcast_room_turbo_stream_refresh(conn, room_id)
  ...
  def delete(conn, %{"id" => id}) do
    ...
    broadcast_room_turbo_stream_refresh(conn, message.room_id, with_request_id: true)
  ...
  defp broadcast_room_turbo_stream_refresh(conn, room_id, opts \\ []) do
    with_request_id = Keyword.get(opts, :with_request_id, false)

    broadcast_turbo_stream_refresh(conn, "turbo_stream:room:#{room_id}", with_request_id: with_request_id)
  end
end
